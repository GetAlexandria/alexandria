...
This package contains the bundled Alexandria plugin payload inside
`alexandria-internal`.

The public repo README is sourced from `../../README.public.md`.
...
- **Installs `ax`** — the public Alexandria CLI
- **Runs plugin setup** — inside a Git repo it installs to
  `.claude/plugins/alexandria/`; outside a repo it falls back to
  `~/.claude/plugins/alexandria/`
- **Prompts for confirmation** before writing anything, showing the install plan
...
```bash
curl -fsSL https://getalexandria.ai/install.sh | ALEXANDRIA_VERSION=0.9.2 bash
```

If `ax` is already installed, the setup step is available directly:
...
This README is the bundled product README. The source-of-truth engineering repo
is `GetAlexandria/alexandria-internal`, where maintainers run:
...
`/ax-library` is the single Raven-led entry point for library configuration. On a
...
Cards are organized into 21 types across four information-flow layers (rationale, product, experience, temporal), presented below in five sections. Each type makes exactly one kind of claim about its content — see `Principle - Each Card Type Makes One Kind of Claim` and `Standard - Type Claim Test` for the discriminator.
...
| Type | Purpose |
|------|---------|
| Decision | Settled past choice |
| Initiative | Active commitment in progress |
...
1. **AI Mode** — How much product decision authority do AI builders have? (No/Low AI → Short-Order Cook → Pair Programmer → Factory)
   This is about product decisions, not whether your implementation pipeline is automated. `/ax-library` can record build-pipeline autonomy separately.
...
After `/ax-plan` writes a ticketed implementation plan, you can run
`/ax-sync-tickets` to preview and confirm syncing that plan's `tickets/`
directory into GitHub issues.

If enablers complete or milestone findings change the remaining work, use
`/ax-revise-plan` to update that plan's `release.md` and affected plan files before
implementation continues.

After implementation finishes, use `/ax-complete-plan` to update that plan's
...
1. validate the repo locally with `bun run check`, `bun test`, and `bash ./packages/deploy/build-tarball.sh`
2. tag `v$(cat VERSION)` from a clean `main`
3. let `.github/workflows/release.yml` create the GitHub Release and publish
   `install.sh` plus site metadata to `https://getalexandria.ai`, and the
   release tarballs plus mirrored `latest-version.txt` to
