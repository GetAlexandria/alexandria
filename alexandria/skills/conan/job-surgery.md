...
**Purpose:** Produce actionable transient handoffs for Sam to execute library fixes.
...
4. **Deliver plan** — Output the complete transient handoff for Sam.
...
with all tables filled in. This surgery plan is a transient handoff, not a checked-in repo
artifact.

**Do not write this plan to disk unless the human explicitly asks for a file.** In normal
Surgery runs, do not call `Write` or create `docs/alexandria/surgery-plan.md` or any other
repo file. The tables MUST appear in your response text, and the inline response is the
deliverable. End your response with `**Status: DONE**` as the literal last line.
...
and also creating `docs/alexandria/surgery-plan.md`

Correct pattern for this job:
- full plan rendered inline in the conversation
- no repo file written
- literal last line is `**Status: DONE**`
