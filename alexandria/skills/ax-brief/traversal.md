...
How to navigate Alexandria's knowledge graph using Claude Code's native tools.
...
| Node ID       | File name (e.g., `System - Notification Engine`)      |
...
```
Glob: docs/alexandria/**/System - Notification Engine.md
```

### By type (all cards of a type)

```
Glob: docs/alexandria/product/systems/*.md
Glob: docs/alexandria/rationale/principles/*.md
Glob: docs/alexandria/experience/loops/*.md
Glob: docs/alexandria/experience/forces/*.md
```

### By topic (keyword search)

```
Grep: pattern="priority queue" path="docs/alexandria/"
```
...
```
Grep: pattern="Progressive Disclosure" path="docs/alexandria/"
```

This finds all cards that reference the Progressive Disclosure principle in any section.

To narrow to a specific dimension header:

```
Grep: pattern="## WHY" path="docs/alexandria/product/systems/" -A 10
```
...
3. Each wikilink has a context phrase explaining the relationship:
   ```
   - [[System - Notification Engine]] — powers the alert delivery layer
   ```
...
```
Grep: pattern="Notification Engine" path="docs/alexandria/"
```

This surfaces every card that references Notification Engine, revealing reverse relationships.
...
3. Parent Section → find its Domain
4. Domain is the top
```

**Pattern:** Component → Template → Section → Domain
...
**Target type:** Template (Kanban Board)
**Task type:** Feature addition

1. **Seed:** `Glob` for `**/Template - Kanban Board.md` → Read it
2. **Parent:** WHERE section shows `[[Section - Project Board]]` → Read it
...
**Result:** ~8 cards covering the Template, its Section, conforming Standards, displayed Primitive, and strategic rationale.
...
3. **Sections:** `Grep` for "System" across `product/sections/` → finds Section - System Board
...
**Result:** ~12 cards covering the Primitive, its associated Section and Capabilities, gap analysis from WHEN sections, and the strategic rationale for why Systems matter.
