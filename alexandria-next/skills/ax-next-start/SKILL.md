...
Use AX2 commands for Alexandria Next state reads and play execution.
...
plugin monitor is always running when Alexandria Next is configured, but it only
...
```bash
ax2 inspect subscriptions list --json
```
...
When `ax2 inspect state --json` reports `raven.vision.status` as `banked`,
...
or trigger internals in the normal greeting. Use AX2 commands to orient
...
For AX2, keep the same shape but shorten it. Use this opener verbatim or very
...
5. After initialization or welcome, inspect projected state with
   `ax2 inspect state --json` before deciding whether a play should run.
