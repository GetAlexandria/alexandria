...
2. **Apply decision tree to each card** (see Conan-Skill Type Taxonomy)
   ...
   Step 2: WHAT + Builders interact?
     - Navigate TO it? Top-level → Domain. Nested → Section.
     - Persistent across domains? → Governance
     - Interact WITHIN? Spatial → Template. Widget → Component.
   ...
   Step 5: User experience over time?
     - Repeating engagement cycle → Loop
     - Multi-phase progression arc → Journey
     - Target emotional experience → Experience Goal
...
4. **Check conformance** — For product-layer cards (Sections, Governance, Templates, Components, Artifacts, Capabilities, Agents):

   Check conformance obligations against the canonical table in `docs/alexandria/reference.md`. Each product-layer card touching a governed domain must link to the constraining Standard listed there.
